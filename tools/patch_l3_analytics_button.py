...
if LABEL in text or HREF in text:
    raise SystemExit("ERROR: L3 workshop analytics button/link already exists in index.html; refusing to patch twice.")

anchor_count = text.count(ANCHOR)
if anchor_count != 1:
    raise SystemExit(f"ERROR: expected exactly one safe anchor in index.html, found {anchor_count}; refusing to guess.")
...
print("Patched index.html: added only the L3 workshop analytics card.")
