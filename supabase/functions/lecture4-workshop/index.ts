...
function gradePart(mode:string,spec:any,response:any){
  ...
  if(mode==='ratio'){const c=String(spec?.canonical??'');const r=normSymbolic(response);if(r===normSymbolic(c))return true;const tgt=ratioNumericTarget(c);if(tgt!==null){const x=parseNumber(response);if(x!==null&&Math.abs(x-tgt)<=1e-9)return true}if(c.includes('=')&&r===normSymbolic(c.split('=',2)[1]))return true;return false}
  return false;
}
